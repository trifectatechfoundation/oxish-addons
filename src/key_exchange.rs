...
use aws_lc_rs::{
    aead::{BoundKey, OpeningKey, UnboundKey, AES_128_GCM},
    agreement::{self, EphemeralPrivateKey, UnparsedPublicKey, X25519},
    cipher::{StreamingEncryptingKey, UnboundCipherKey, AES_128},
    digest, hmac,
...
use crate::{
    proto::{with_mpint_bytes, AesGcmNonce, Decode, Decoded, Encode, MessageType, Packet},
...
impl EcdhKeyExchange {
    pub(crate) async fn advance(
        ...
        conn.stream_read.set_opening_key(OpeningKey::new(
            UnboundKey::new(
                &AES_128_GCM,
            ...
            .unwrap(),
            AesGcmNonce::from_iv(
                raw_keys.client_to_server.initial_iv.as_ref()[..12]
                    .try_into()
                    .unwrap(),
            ),
        ));
